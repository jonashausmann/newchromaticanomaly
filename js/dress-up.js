...
(function () {
  ...
  function init() {
    ...
    if (isMobile) {
      items.forEach(function (el) {
        ...
        el.addEventListener('click', onMobileItemClick);
      });
    } else {
      items.forEach(function (el) {
  ...
  function onMobileItemClick(e) {
    ...
    var zoneMap = { dress: 'zone-dress', shirt: 'zone-shirt', bottom: 'zone-bottom', boots: 'zone-boots' };
    var zone = document.getElementById(zoneMap[category]);
    if (!zone) return;
    ...
    div.addEventListener('click', onMobilePlacedClick);
    ...
    document.querySelectorAll('.clothing-item.mobile-selected').forEach(function (el) {
      el.classList.remove('mobile-selected');
    });
    item.classList.add('mobile-selected');
    setTimeout(function () { item.classList.remove('mobile-selected'); }, 300);
  }

  function onMobilePlacedClick(e) {
    e.stopPropagation();
    var placedItem = e.currentTarget;
    placedItem.remove();
    updateDressZoneTop();
  }
